...
(def nonblank-string [:string {:min 1}])
(def source-record-id [:or :int nonblank-string])
(def numeric [:or :int :double])
...
(defn- iso-date->epoch-ms [value]
  #?(:clj
     ...
     :cljs
     (let [date (js/Date. (str value "T00:00:00.000Z"))
           epoch (.getTime date)]
       (when-not (js/isNaN epoch) epoch))))
...
(defn- normalize-date [value]
  (cond
    ...
    (let [integer-value
          ...
          iso-value (iso-date->epoch-ms value)]
      (or integer-value
          iso-value
          (throw (ex-info "PortWatch date string must be epoch milliseconds or ISO yyyy-MM-dd"
                          {:river-city/error :invalid-portwatch-date
                           :source/date value}))))

    :else
    (throw (ex-info "PortWatch date must be epoch milliseconds or ISO yyyy-MM-dd"
                    {:river-city/error :invalid-portwatch-date
                     :source/date value}))))

(defn target-chokepoint? [port-name]
  (boolean
   (and port-name
        (re-find #"(?i)(hormuz|bab[ -]?el[ -]?mandeb|mandeb)" port-name))))

(defn normalize-attributes
  "Convert one ArcGIS PortWatch attribute map into River City's portable data shape."
  [attrs]
  {:source/id source-id
...
(defn valid-observation? [value]
  (m/validate observation-data value))

(defn assert-observation! [value]
